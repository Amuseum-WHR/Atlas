...
class ShapeNet(data.Dataset):
    ...
    def __init__(self, opt, train=True):
        ...
        if not opt.demo:
            ...
            dataset_path = '/mnt/d/Mr.Wang/大学/大三上/计算机视觉/Atlas/AtlasNet-master'

            # Load classes
            self.pointcloud_path = join(dataset_path, 'data/ShapeNetV1PointCloud')
            self.image_path = join(dataset_path, 'data/ShapeNetV1Renderings')

            # Load taxonomy file
            self.taxonomy_path = join(dataset_path, 'data/taxonomy.json')
            ...
            # Create Cache path
            self.path_dataset = join(dataset_path, 'data', 'cache')
